...
extension ApiError {
   public var errorDescription: String {
      ...
      case .noResponseReceived(let error):
         if let error {
            return
               "The request was sent, but the server response was not received. Please check your internet connection. System error: \(error.localizedDescription)"
         } else {
      ...
      case .noDataInResponse(let statusCode):
         return
            "The request was sent and the server responded with status code \(statusCode), but the response did not include any body although a body was requested."

      case .responseDataConversionFailed(let type, let error):
         return
            "The request was sent and the server responded with a body, but the conversion of the body to the expected type \(type.description) failed. System error: \(error.localizedDescription)"
